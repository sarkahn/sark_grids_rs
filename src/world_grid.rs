...
impl WorldGrid {
    pub fn new(world_pos: (f32, f32), size: [u32;2], pivot: Pivot) -> Self {
        ...
        let center_offset = match pivot {
            Pivot::Center => Vec2::select((size % 2).cmpeq(UVec2::ZERO), Vec2::new(0.5,0.5), Vec2::ZERO),
            _ => Vec2::new(0.5,0.5),
        };
    ...
    pub fn try_grid_to_index_2d(&self, grid_pos: [i32;2]) -> Option<UVec2> {
        ...
        if index.cmpge( Vec2::ZERO ).all() && 
           index.cmplt( self.size.as_vec2() ).all() {
    ...
    fn iter(&self, offset: Vec2, axis: Vec2) -> impl Iterator<Item = Vec2> {
        (0..self.height()).cartesian_product(0..self.width()).map(
            move |(y,x)| (Vec2::new(x as f32, y as f32) + offset) * axis
        )
    }
}
